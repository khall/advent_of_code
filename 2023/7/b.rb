...
class CamelCards
  ...
  def get_poker_hands_by_type
    ...
    @input.each do |hand, bid|
      ...
      jokers = tally.find { |card, _count| card == 'J' }
      num_jokers = jokers.nil? ? 0 : jokers[1]
      tally.reject! { |arr| arr[0] == 'J' }
      case tally.dig(0, 1).to_i + num_jokers
      when 5
        # five of a kind
        hands[:five] << [hand, bid]
      when 4
  ...
  def order_hands(hands)
    hands.each do |hand_type, hands_of_type|
      hands[hand_type] = hands_of_type.map { |hand, bid| [hand.tr('A', 'E').tr('K', 'D').tr('Q', 'C').tr('J', '1').tr('T', 'A'), bid] }.sort { |hand_arr_a, hand_arr_b| hand_arr_a[0] <=> hand_arr_b[0] }
    end
  end
