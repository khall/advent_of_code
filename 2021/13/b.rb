...
class Origami
  ...
  def fold
    @instructions.each do |axis, value|
      if axis == 'x'
    ...
    @map.each do |row|
      return if row.index('X') == nil
      last_one = row.rindex('X')
      puts row[0..last_one].join('')
    end
  end
  ...
  def fold_along_column(fold_column)
    @map.each_with_index do |row, i|
      row.each_with_index do |_cell, j|
        ...
        @map[i][j] = @map[i][fold_column * 2 - j] if @map[i][fold_column * 2 - j] != ' '
  ...
  def fold_along_row(fold_row)
    @map.each_with_index do |row, i|
      ...
      row.each_with_index do |_cell, j|
        @map[i][j] = @map[fold_row * 2 - i][j] if @map[fold_row * 2 - i][j] != ' '
